...
const SellingNow = () => {
  ...
  const fetchProperties = async () => {
    ...
    try {
      const response = await fetch(
        'https://sqb4dkpp.api.sanity.io/v2023-09-17/data/query/production?query=*[_type == "offers"]{name, location, purpose, size, cashPrice, installmentPrice, "imageUrl": image.asset->url, slug}'
      );
  ...
  return (
    <div className={styles['selling-now']}>
      <h2>EXCLUSIVE OFFERS ON PREMIUM PROPERTIES</h2>
